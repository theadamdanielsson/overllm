...
# The Vercel functions are only detected when imported from "ai".
IMP = 'import { generateText, streamText, generateObject } from "ai";\n'
...
def test_detects_vercel_generate_text():
    assert n_calls(IMP + 'const r = await generateText({ model, prompt: "hi there friend" });') == 1
...
def test_local_generatetext_not_flagged():
    # user-defined helper named generateText, NOT the Vercel SDK (no import from "ai")
    src = "function generateText(p, opts) { return call(p); }\nconst r = await generateText(prompt, { provider });"
    assert n_calls(src) == 0


def test_vercel_extraction_flagged():
    src = IMP + 'const r = await generateText({ model, prompt: "Extract the email from " + x });'
...
def test_variable_held_prompt_resolved():
    src = IMP + "const p = `Extract the email from ${x}`;\nconst r = await generateText({ model, prompt: p });"
    assert "llm-extraction" in rule_set(src)


def test_static_prompt_flagged():
    assert "static-prompt" in rule_set(IMP + 'const r = await generateText({ model, prompt: "Write a haiku about the sea." });')


def test_batchable_for_of_loop_flagged():
    src = IMP + "for (const row of rows) { await generateText({ model, prompt: `tag ${row}` }); }"
    assert "llm-in-loop" in rule_set(src)


def test_while_loop_is_necessary():
    assert "llm-in-loop" not in rule_set(IMP + "while (!done) { await generateText({ model, prompt: p }); }")


def test_streaming_for_await_not_a_loop():
    src = IMP + "for await (const chunk of streamText({ model, prompt: p })) { use(chunk); }"
    assert "llm-in-loop" not in rule_set(src)


def test_web_request_prompt_injection():
    src = IMP + "const q = req.query.q;\nconst r = await generateText({ model, prompt: `Answer: ${q}` });"
    assert "prompt-injection" in rule_set(src)


def test_normal_dynamic_is_silent():
    assert rule_set(IMP + 'const r = await generateText({ model, prompt: `Summarize: ${article}` });') == set()


def test_jsx_and_tsx_parse():
    assert n_calls(IMP + 'const r = await generateText({ model, prompt: "hello world here" });', "tsx") == 1
    assert n_calls(IMP + 'const r = await generateText({ model, prompt: "hello world here" });', "js") == 1
